        ...
        if path == "/":
            with db_conn() as conn:
                stats = {
                    ...
                    "inventory": conn.execute("SELECT COUNT(*) AS c FROM inventory").fetchone()["c"],
                    "batches": conn.execute("SELECT COUNT(*) AS c FROM production_batches").fetchone()["c"],
                ...
                recent = conn.execute(
                    "SELECT flavor, volume_liters, status, operator_name FROM production_batches ORDER BY id DESC LIMIT 5"
                ).fetchall()

            cards = "".join(
                f"<article class='card'><h3>{k}</h3><p>{v}</p></article>"
                for k, v in {
                    "Сотрудники": stats["employees"],
                    "Позиции склада": stats["inventory"],
                    "Партии": stats["batches"],
                    "Заказы": stats["orders"],
                }.items()
            )
...
if __name__ == "__main__":
    try:
        init_db()
        seed_data()
    except Exception as exc:  # startup visibility only
